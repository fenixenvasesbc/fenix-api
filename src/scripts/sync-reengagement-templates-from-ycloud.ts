...
const MAX_RETRIES = 3;
const CAMPAIGN_KEY_PREFIX = 'week1_reengagement';
const CAMPAIGN_NAME = 'Reenganche semana 1';
...
function printSummary(summary: Summary, apply: boolean) {
  console.log('\nYCloud reengagement templates sync summary');
...
async function main() {
  ...
  console.log(
    `Starting YCloud reengagement template sync mode=${args.apply ? 'APPLY' : 'DRY RUN'} templateName=${args.templateName} limit=${args.limit} delayMs=${args.delayMs}`,
  );
...
void main().catch((error) => {
  console.error(`YCloud reengagement template sync failed: ${errorMessage(error)}`);
  process.exitCode = 1;
});
