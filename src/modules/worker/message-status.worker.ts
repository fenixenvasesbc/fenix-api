...
import { MessageStatusService } from '../message-status/message-status.service';
import type { WebhookInboxJob } from 'src/common/types/webhook-inbox-job';

@Injectable()
export class MessageStatusWorker implements OnModuleInit {
  private readonly logger = new Logger(MessageStatusWorker.name);
  ...
  async onModuleInit() {
    const queue = process.env.RABBITMQ_QUEUE_MESSAGE_UPDATED!;
    this.logger.log(`Starting message-status consumer on queue=${queue}`);
  ...
  private async handleMessage(msg: ConsumeMessage): Promise<ConsumeDecision> {
    ...
    try {
      ...
      this.logger.error(
        `MessageStatus worker failed. deaths=${deaths}. action=${decision.action}:${target}. err=${String(err)}`,
      );
