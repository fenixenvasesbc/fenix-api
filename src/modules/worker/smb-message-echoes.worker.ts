...
import { SmbMessageEchoesService } from '../smb-message-echoes/smb-message-echoes.service';

@Injectable()
export class SmbMessageEchoesWorker implements OnModuleInit {
  private readonly logger = new Logger(SmbMessageEchoesWorker.name);

  constructor(
    private readonly rabbit: RabbitmqService,
    private readonly smbMessageEchoesService: SmbMessageEchoesService,
  ) {}
  ...
  async onModuleInit() {
    ...
    this.logger.log(`Starting SMB message echoes consumer on queue=${queue}`);
  ...
  private async handleMessage(msg: ConsumeMessage): Promise<ConsumeDecision> {
    ...
    try {
      ...
      await this.smbMessageEchoesService.process(job);
      ...
      if (job) {
        await this.smbMessageEchoesService.markFailed(
          job,
          err,
      ...
      this.logger.error(
        `SMB message echoes worker failed. deaths=${deaths}. action=${decision.action}:${target}. err=${String(err)}`,
      );
