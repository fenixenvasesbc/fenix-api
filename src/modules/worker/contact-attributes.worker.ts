...
import { ContactAttributesService } from '../contact-attributes/contact-attributes.service';

@Injectable()
export class ContactAttributesWorker implements OnModuleInit {
  private readonly logger = new Logger(ContactAttributesWorker.name);

  constructor(
    private readonly rabbit: RabbitmqService,
    private readonly contactAttributesService: ContactAttributesService,
  ) {}
  ...
  async onModuleInit() {
    ...
    this.logger.log(`Starting contact-attributes consumer on queue=${queue}`);
  ...
  private async handleMessage(msg: ConsumeMessage): Promise<ConsumeDecision> {
    ...
    try {
      ...
      await this.contactAttributesService.process(job);
      ...
      if (job) {
        await this.contactAttributesService.markFailed(
          job,
          err,
      ...
      this.logger.error(
        `ContactAttributes worker failed. deaths=${deaths}. action=${decision.action}:${target}. err=${String(err)}`,
      );
