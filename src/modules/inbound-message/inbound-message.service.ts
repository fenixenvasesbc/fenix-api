...
export class InboundMessageService {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!lead.firstInboundAt || !lead.respondedAt) {
        await tx.lead.update({
          where: { id: lead.id },
          data: {
            firstInboundAt: lead.firstInboundAt ?? inboundAt,
      ...
      const message = await tx.message.upsert({
        ...
        update: {
          providerCreateTime: inbound.providerCreateTime ?? undefined,
          providerSendTime: inbound.providerSendTime ?? undefined,
          contextWamid: inbound.contextWamid ?? undefined,
          textBody: inbound.textBody,
          mediaUrl: inbound.mediaUrl,
          mimeType: inbound.mimeType,
          caption: inbound.caption,
  ...
  private normalizeInbound(payload: YCloudInboundPayload): NormalizedInbound {
    ...
    if (!msg) throw new Error('Missing whatsappInboundMessage');
