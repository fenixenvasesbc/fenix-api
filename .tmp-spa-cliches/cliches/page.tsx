...
export default function ClichesPage() {
  ...
  const [categories, setCategories] =
    useState<ClicheCategory[]>(DEFAULT_CATEGORIES);
  ...
  const [form, setForm] = useState<ClicheForm>(() =>
    emptyForm(DEFAULT_CATEGORIES),
  );
  ...
  const selectedYear = useMemo(() => {
    ...
    return Number.isInteger(parsed) && parsed >= 1900 && parsed <= 9999
      ? parsed
      : undefined;
  }, [year]);
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'No se pudieron cargar los cliches',
      );
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    clichesApi
      .categories()
      .then((values) => {
        if (values.length) setCategories(values);
      })
      .catch(() => undefined);
  }, []);
  ...
  function buildPayload(): ClichePayload | null {
    ...
    if (
      ...
      setFormError('Introduce un ano valido');
      return null;
    }
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFormError(
        err instanceof Error ? err.message : 'No se pudo guardar el cliche',
      );
    } finally {
      setIsSaving(false);
    }
  ...
  async function removeCliche() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'No se pudo eliminar el cliche',
      );
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="flex flex-col gap-6">
                ...
                <TableCell colSpan={6} className="h-28 text-center">
                  <div className="flex flex-col items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-sm">
                      No hay cliches con estos filtros
                    </span>
                  </div>
                </TableCell>
              </TableRow>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing ? 'Editar cliche' : 'Nuevo cliche'}
            </DialogTitle>
            <DialogDescription>
              {editing
                ? 'Actualiza los datos del registro.'
                : 'Registra un cliche de fabricacion.'}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submitForm} className="grid gap-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="cliche-year">Ano</Label>
              <Input
                id="cliche-year"
                type="number"
                min={1900}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se eliminara {pendingDelete?.name}. Esta accion no se puede
              deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
